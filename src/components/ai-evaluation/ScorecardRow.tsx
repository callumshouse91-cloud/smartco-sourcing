...
function badgeClass(color: 'green' | 'amber' | 'red') {
  if (color === 'green') return 'bg-mint-500/10 text-mint-500 border-mint-300/40'
  if (color === 'amber') return 'bg-amber-500/10 text-amber-600 border-amber-500/30'
  return 'bg-coral-500/10 text-coral-500 border-coral-500/30'
}
...
export default function ScorecardRow({ cards }: { cards: ScoreCard[] }) {
  ...
  return (
    <FadeIn>
      <div className="grid grid-cols-2 gap-4 md:grid-cols-3 xl:grid-cols-6">
        {cards.map((card) => (
          <div key={card.category} className="card p-4">
            ...
            {card.type === 'badge' ? (
              ...
              <>
                ...
                <AnimatedProgressBar
                  ...
                  className="mt-2 h-1.5 overflow-hidden rounded-full bg-[#e0e4ec]"
                />
              </>
            )}
          </div>
